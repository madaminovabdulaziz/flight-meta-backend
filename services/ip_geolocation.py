# services/ip_geolocation.py
"""
Get user's nearest airport by IP address
No database required - uses in-memory major airports data
"""
...
class IPGeolocationService:
    ...
    async def get_location_from_ip(self, ip: str) -> Optional[Dict]:
        ...
        Returns: {"city": "London", "country": "GB", "lat": 51.5, "lon": -0.1}
        """
    ...
    def _get_default_location(self) -> Dict:
        """Default to Tashkent for failed/local requests"""
        return {
            "city": "Tashkent",
            "country": "UZ",
            "lat": 41.2995,
    ...
    def find_nearest_airport(self, lat: float, lon: float, country: str = None) -> Dict:
        ...
        for iata, airport in self.MAJOR_AIRPORTS.items():
            ...
            # Bonus for same country (reduces effective distance)
            if country and airport["country"] == country:
                distance *= 0.7  # 30% preference for domestic airports
            
            # Bonus for popularity (major hubs)
